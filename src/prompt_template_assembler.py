...
class PromptTemplateAssembler:
    def __init__(self, yaml_path=None, logger=None):
        ...
        if yaml_path:
            self.load_preprompt_skeletons_from_yaml(yaml_path)

    def bring(self, name):

        for idx, p in enumerate(self.prompt_objects):
            if p.name == name:
                return p
    ...
    def load_preprompt_skeletons_from_yaml(self, yaml_file_path):

        yaml_data = load_yaml_file(yaml_file_path)

        self.prompt_objects = self.create_prompt_objects(yaml_data)

        # self.logger.info("prompts are loaded from yaml. ", lvl=6)

        # for e in self.prompt_objects:
        #     print("  >>>>>>>>>> ", e.name,  e.placeholder)

    def bring_preprompts(self, list_of_preprompt_names):
        p = []

        prompt_object_names=[p.name for p in self.prompt_objects]
       # self.logger.info("prompt_object names: %s ", prompt_object_names, lvl=12)
        for name in list_of_preprompt_names:
            #self.logger.info("prompt name: %s ", name, lvl=13)
            r = self.bring(name)
            if not r:
                raise Exception(f"Preprompt '{name}' not found.")
            p.append(r)
        return p
    ...
    def add_preprompts_to_bin(self, list_of_preprompts):
       # self.logger.info("list_of_preprompts: %s ", list_of_preprompts, lvl=12)
        if self.is_list_of_strings(list_of_preprompts):
            list_of_preprompts = self.bring_preprompts(list_of_preprompts)

        for e in list_of_preprompts:
            self.bin.add(e)

    def craft(self, placeholder_dict, preprompts=None, ):

        if preprompts:
            self.add_preprompts_to_bin(preprompts)
        final_preprompt = self.merge_simple(self.bin.storage, placeholder_dict)
    ...
    def merge_simple(self, prompt_objects, placeholder_dict=None):

        prompt = ""
        prompt_objects_constructed = [prompt.construct_statement() for prompt in prompt_objects]

        formatter = string.Formatter()

        # Dynamically build the unformatted prompt using the constructed informative pieces
        for piece in prompt_objects_constructed:
    ...
    def add_all_preprompts_to_bin(self):
        self.bin.storage=  self.prompt_objects
    ...
    def craft_question_using_category(self, data_for_placeholders,  question_category, supporter_category=None):
        ...
        deactive_questinables = [prompt for prompt in questinables if prompt != question]
        # deactive_questinables_constructed = [prompt.construct_statement() for prompt in deactive_questinables]

        prompt = ""
        if supporter_category:
            supporter_category_preprompts = self.bin.bring_by_category(supporter_category)

            supporter_category_preprompts_constructed = [prompt.construct_statement() for prompt in supporter_category_preprompts]
            for e in supporter_category_preprompts_constructed:
                prompt += f"{e}\n\n"

        # prompt = f"{question_constructed}\n\n"

        prompt += self.merge_simple(deactive_questinables, data_for_placeholders)
    ...
    # def craft_question(self,category_filter_for_questionable, context_data):
    #
    #     self.bin.find_questionable_prompts()
    #     question=None
    #     for e in self.bin.questinables:
    #         # print("     e: ", e.name )
    #         if e.category==category_filter_for_questionable:
    #             if e.marked_as_question==False:
    #                 if e.answered==False:
    #                     print("     e: ", e.name)
    #                     e.make_question()
    #                     question=e
    ...
    #     final_preprompt=self.arrange_prompts(self.bin.storage, context_data)
    #     question.close_question()
    #     return final_preprompt,question


    def dynamic_prompt_maker(self, prompt_name, prompt_text):

        prompt_object = Preprompt(
            name=prompt_name,
            info=prompt_text,
            category="dynamic"
        )
    ...
    def create_prompt_objects(self, yaml_data):
        """
        ...
        for category, data in yaml_data.items():
            for prompt_data in data:
                prompt_object = Preprompt(
                    ...
                    statement_suffix= prompt_data.get("statement_suffix", "") ,  # Assign 'self.statement_suffix'
                    placeholder_proclamation= prompt_data.get("placeholder_proclamation", None) ,
                    placeholder= prompt_data.get("placeholder", None) ,
                    info=prompt_data.get("info", None),
                    category=category
                )
    ...
    def build_informative_prompt(self, informative_prompts_constructed, context_data):
        """
        ...
        # Dynamically build the unformatted prompt using the constructed informative pieces
        for piece in informative_prompts_constructed:
            # Extract the field names from the piece using the string formatter
            field_names = [field_name for _, field_name, _, _ in formatter.parse(piece) if field_name]
    ...
    def merge_by_category(self, category, context_data, filter_list=None):
        prompt_objects = self.bin.storage
       ...
       # print("merge_by_category---------------------------")
        ...
        same_category_prompt_objects = [prompt for prompt in prompt_objects if prompt.category in category]


        if filter_list:
            filtered_same_category_prompt_objects = [prompt for prompt in same_category_prompt_objects if prompt.name not in filter_list]
        else:
            filtered_same_category_prompt_objects = same_category_prompt_objects
        ...
        prompt = " "
        prompt +=self.merge_simple( filtered_same_category_prompt_objects, placeholder_dict=context_data)
    ...
    def gather_informative(self,  context_data, filter_list=None, dont_bring_guide=False):
        ...
        informative_prompts = [prompt for prompt in prompt_objects if prompt.category != "command_prompts"]
        informative_prompts = [prompt for prompt in informative_prompts if prompt.name != "step_guide"]
        if filter_list:
            filtered_informative_prompts = [prompt for prompt in informative_prompts if prompt.name not in filter_list]
        else:
            filtered_informative_prompts=informative_prompts
        informative_prompts_constructed = [prompt.construct_statement() for prompt in filtered_informative_prompts]
    ...
    def arrange_prompts(self, prompt_objects, context_data):
        """
        ...
        for idx, p in enumerate(prompt_objects):
        ...
        if question:
            # Remove the question prompt from the list to only have the informative prompts remaining
            informative_prompts = [prompt for prompt in prompt_objects if prompt != question]
            ...
            question_constructed = question.construct_statement()
            informative_prompts_constructed = [prompt.construct_statement() for prompt in informative_prompts]
    ...
    def create_prompt(self, question, prompt_pieces, context_data):
        """
